...
from .forms import CardForm, DeckForm, CardFormSet
...
def delete_deck(request, deck_id):
    ...
    return redirect('flashcards:all_decks')
...
def create_cards(request, deck_id):
    ...
    if request.method == 'POST':
        ...
        if formset.is_valid():
            formset.save()
            return redirect('flashcards:create_cards', deck_id)
...
def save_for_study(request, deck_id):
    ...
    return redirect('flashcards:show_my_studyset')
...
def flip(request, deck_id):
    deck = Deck.objects.get(pk=deck_id)
    c_list = Card.objects.filter(deck_id=deck)
    context = {'list_of_cards' : c_list}

    return render(request, 'flip_deck.html', context)
